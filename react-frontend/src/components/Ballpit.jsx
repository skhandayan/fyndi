...
import { BoxGeometry } from 'three'; // Add this import at the top (if not already imported)
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    const o = new BoxGeometry(); // Replace SphereGeometry with BoxGeometry
    ...
    this.rotationData = new Float32Array(this.count * 3).fill(0);
    this.rotationSpeeds = new Float32Array(this.count * 3);

    // Initialize random rotation speeds
    for (let i = 0; i < this.count; i++) {
      const base = i * 3;
      this.rotationSpeeds[base] = Math.random() * 0.01 + 0.005;     // X
      this.rotationSpeeds[base + 1] = Math.random() * 0.02 + 0.01;   // Y
      this.rotationSpeeds[base + 2] = Math.random() * 0.015 + 0.005; // Z
    }
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0 && this.config.followCursor === false) {
        ...
        U.scale.set(size, size, size);
      }

      // Rotate based on per-instance speed
      this.rotationData[base] += this.rotationSpeeds[base];
      this.rotationData[base + 1] += this.rotationSpeeds[base + 1];
      this.rotationData[base + 2] += this.rotationSpeeds[base + 2];

      U.rotation.set(
        this.rotationData[base],
        this.rotationData[base + 1],
        this.rotationData[base + 2]
      );
